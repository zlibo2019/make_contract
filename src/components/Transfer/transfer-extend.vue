    ...
    <div class="transfer" :style="{width,height}">
        ...
        <div class="transfer-left">
            <h3 class="transfer-title">
                ...
                <span class="u-right">{{leftTitle}}</span>
            </h3>
            <!-- 内容区 -->
            <div class="transfer-main">
                <!-- <slot name="from"></slot> -->
                <el-autocomplete class="inline-input" v-model="mySearch" :fetch-suggestions="querySearch"
                        :placeholder="leftplaceholder" :trigger-on-focus="false" @select="handleSelect">
                    <template slot-scope="{ item }">
                        <span>{{ item.value }}</span>
                        <span style="float:right">{{ item.pname }}</span>
                    </template>
                </el-autocomplete>
        ...
        <div class="transfer-right">
            <h3 class="transfer-title">
                <span class="u-right" v-if="differ == 1">已选择（{{ studentSelection.length }}）人</span>
                <span class="u-right" v-if="differ == 2">已选择（{{ studentSelection.length }}）个</span>
            </h3>
            <!-- 内容区 -->
            <div class="transfer-main">
                <el-table id="cardTable" ref="cardTable" border height="100%" :data="transferData" tooltip-effect="dark"
                    ...
                    <el-table-column prop="studentName" label="姓名" align="center" v-if="differ == 1"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column prop="className" label="班级" align="center" v-if="differ == 1"
                                     column-key="className" show-overflow-tooltip></el-table-column>
									 <!-- TODO -->
                    <el-table-column prop="buildingName" :label="rightOne" align="center" v-if="differ == 2"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column prop="placeName" :label="rightTwo" align="center" v-if="differ == 2"
...
//				to_check_keys: [], // 目标数据选中key数组 以此属性关联穿梭按钮，总全选、半选状态
				...
				fuzzyArr:[],         //模糊查询数组
				differ:this.sourceDiffer,//1 选择学生， 2选择教室
			};
		},
		props: {
			// 宽度
		...
		methods: {
			...
			handleSelect(item) {
				...
				this.mySearch = '';
				if(this.differ == 1){
					this.transferData.push({"id":item.id,"label":item.value,"className":item.pname,"disabled":'true',"pid":item.pid,"userNo":item.userNo,"studentName":item.value});
				...
				}else if(this.differ == 2){
					this.transferData.push({"id":item.id,"label":item.value,"placeName":item.pname,"disabled":'true',"pid":item.pid,"buildingName":item.value});
			...
			addToAims() {
				...
				if(this.differ == 1){
					...
					for (let k = 0; k < checkArr.length; k++) {
						...
						checkArr[k].pname = getNode.parent.parent.label+' '+getNode.parent.label
						checkArr[k].disabled = true;
					}
				}else if(this.differ == 2){
					for (let g = 0; g < getCheckedNodes.length; g++) {
						let getNode = this.$refs["from-tree"].getNode(getCheckedNodes[g]);
						console.log(getNode.data.group,getCheckedNodes[g])
						// if(getNode.data.group == 104){//只选教室
							checkArr.push(getCheckedNodes[g])
						// }
					}
					// console.log(checkArr)
        ...
        if(this.differ == 1){
          ...
          function ifArrVal(arr, value) {//多维数组判断是否存在某值
            for (let i = 0; i < arr.length; i++) {
                  // console.log(666666, arr[i].id , value)
              if (arr[i].children instanceof Array) {//判断是否为多维数组
                if (arr[i].children.length > 0) {
                  ifArrVal(arr[i].children, value);
                } else {
          ...
          console.log(1111,this.transferData)
          // let checkKeys = this.transferData.map(item=>{
          //   return item.id
          // });
          // console.log(221,checkKeys);
          // this.$refs["from-tree"].setCheckedKeys(checkKeys);
			...
			loadAll() {
				...
				let vThis = this;
				let classArr = [];
				function ifArrVal(arr, value) {//多维数组判断是否存在某值  选择学生
					for (let i = 0; i < arr.length; i++) {
						if (arr[i].children instanceof Array) {//判断是否为多维数组
							if (arr[i].children.length > 0 ) {
					...
					for (let i= 0; i<vThis.fuzzyArr.length;i++){
						vThis.fuzzyArr[i].pname=classArr[i]
                    }
					return -1;//不存在
				}
				function ifArrVal2(arr, value) {//多维数组判断是否存在某值  选择教室
					for (let i = 0; i < arr.length; i++) {
						if (arr[i].children instanceof Array) {//判断是否为多维数组
							if (arr[i].children.length > 0 ) {
								if(arr[i].group == 103){
									for(let k=0;k<arr[i].children.length;k++){
										// if(arr[i].children[k].children.length == 0){
										if(arr[i].children[k].group == 104){
											// console.log(`'楼宇：'${arr[i].label}--'名称：'${arr[i].children[k].label}`,arr[i].children[k])
											let reg = new RegExp(value);
											if(arr[i].children[k].label.match(reg)){
												classArr.push(arr[i].label)
												// console.log(6666666666666,classArr)
											}
											/* 多校区使用 grounp都要 +1 */
											// for(let j = 0 ;j<arr[i].children[k].children.length;j++){
											// 	console.log(777777777777,arr[i].children[k].children[j].label,value)
											// 	let reg = new RegExp(value);
											// 	if(arr[i].children[k].children[j].label.match(reg)){
											// 		classArr.push(arr[i].label+' '+arr[i].children[k].label)
											// 		console.log(6666666666666,classArr)
								...
								ifArrVal2(arr[i].children, value);
							} else {
								let reg = new RegExp(value);
								if(arr[i].label.match(reg)){
									// console.log(787878,arr[i])
									vThis.fuzzyArr.push({"value":arr[i].label,"label":arr[i].label,"id":arr[i].id,"pid":arr[i].pid});
								}
							}
						}
					}
				...
				if(this.differ == 1){
					...
					ifArrVal2(this.self_from_data, this.mySearch)
				}
